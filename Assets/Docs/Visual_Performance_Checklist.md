...
- Run `Cementery > Performance > Start Visual Evidence Route` to cycle controlled day clear, sunset haze, night fog, and dawn fog states while the sampler is recording. The route can be launched from Edit Mode and writes phase screenshots to `Application.persistentDataPath/visual-evidence-route` plus a profiler capture to `Application.persistentDataPath/visual-evidence-route.raw`.
- Unity Profiler recording with CPU Timeline, Rendering, Memory, and GC Alloc columns visible. The automated profiler capture proves a recording artifact exists; still inspect it manually for final CPU/GPU diagnosis.
...
- In the Unity Editor, run `Cementery > Performance > Generate Visual Performance Report` to convert the latest CSV session into `Assets/Docs/Visual_Performance_Last_Run.md`. The report must show passing route coverage for evidence duration, day/night samples, fog samples, route screenshots, and profiler capture before it can support visual-readability or visual-cost claims.
...
- `Assets/Scripts/Rendering/VisualEvidenceRouteDriver.cs` is an opt-in route driver for repeatable day/sunset/night-fog/dawn sample coverage, phase screenshots, profiler capture, and original visual-state restoration when finished.
...
- `Cementery > Performance > Start Visual Evidence Route` can be launched from Edit Mode; it queues the route, enters Play Mode, starts sampling after Play Mode is active, captures phase screenshots, records a profiler log, and requests report generation when the route finishes.
- `WorldStreamer` keeps the base square loading range but adds a forward predictive band controlled by `forwardExtraRange`, `forwardBandHalfWidth`, `predictiveStreamingMinSpeed`, and `forwardPriorityBias`; validate this with RV traversal because it intentionally improves vision distance in the travel direction before side/rear chunks.
