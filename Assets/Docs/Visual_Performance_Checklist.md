...
- In the Unity Editor, run `Cementery > Performance > Generate Visual Performance Report` to convert the latest CSV session into `Assets/Docs/Visual_Performance_Last_Run.md`. The report must show passing route coverage for evidence duration, day/night samples, fog samples, and route screenshots before it can support visual-readability claims.
