...
namespace Cementery.Rendering
{
    public sealed class VisualPerformanceSampler : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                File.WriteAllText(_samplePath, "time,scene,average_ms,p95_ms,worst_ms,cpu_frame_ms,gpu_frame_ms,sample_count,total_memory_mb,memory_delta_mb\n");
            }
        }
        ...
        private void WriteReport()
        {
            ...
            string line = $"{Time.realtimeSinceStartup:F2},{sceneName},{averageMs:F2},{p95Ms:F2},{_worstFrameMs:F2},{cpuFrameMs:F2},{gpuFrameMs:F2},{_sampleCount},{totalMemoryMb:F2},{memoryDeltaMb:F2}\n";
            File.AppendAllText(_samplePath, line);
            Debug.Log($"VisualPerformanceSampler: avg {averageMs:F2} ms, p95 {p95Ms:F2} ms, worst {_worstFrameMs:F2} ms, CPU {cpuFrameMs:F2} ms, GPU {gpuFrameMs:F2} ms, samples {_sampleCount}, csv {_samplePath}");
        }
#endif
    }
}
