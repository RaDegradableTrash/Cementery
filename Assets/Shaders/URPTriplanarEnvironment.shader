    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(Varyings input) : SV_Target
            {
                ...
                float3 specColor = mainLight.color * sandSpec * mainLight.distanceAttenuation * mainLight.shadowAttenuation;

                // 🌟 Second Layer: Stylized Fresnel rim lighting (风之旅人标志性偏振边缘发光)
                float smoothFresnel = pow(1.0 - saturate(dot(normalWS, viewDir)), 4.0);
                float3 fresnelGlow = mainLight.color * smoothFresnel * 0.28 * saturate(dot(mainLight.direction, normalWS)) * mainLight.shadowAttenuation;
                ...
                float3 sparkleColor = float3(1.0, 0.86, 0.52) * glint * 4.5 * mainLight.shadowAttenuation;
